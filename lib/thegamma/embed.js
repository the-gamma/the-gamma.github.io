...
function loadTheGamma() {
  ...
  require(["monaco", "https://thegamma.net/lib/thegamma/thegamma.js"], function (_, g) {      
    for(var i=0; i<thegamma.length; i++) {
      var id = thegamma[i];
      ...
      var code = document.getElementById(id + "-code").innerText;
...
function initTheGamma() {
  for(var i=0; i<thegamma.length; i++) {
    ...
    el.innerHTML = 
      ('<div id="thegamma-[ID]-out"><p class="placeholder">Yo</p></div>' +
      "<button onclick='openDialog(\"[ID]\")'>Edit</button>" +
...
if (!thegammaInit) { thegammaInit=true; initTheGamma(); }
